...
public class GetWordsQueryHandlerTests
{
    ...
    {
        // ARRANGE
        _wordConceptRepository.Setup(r => r.GetPagedAsync("A1", PartOfSpeech.Noun, null, 1, 20, It.IsAny<CancellationToken>()))
            .ReturnsAsync(new PagedResult<WordConceptAggregate> { Items = [], TotalCount = 0, Page = 1, PageSize = 20 });
        var handler = CreateHandler();

        // ACT
        await handler.Handle(new GetWordsQuery("A1", "Noun", null, 1, 20), default);

        // ASSERT — "Noun" string'i PartOfSpeech.Noun enum'ına DOĞRU parse edilip repository'ye geçirildi
        _wordConceptRepository.Verify(r => r.GetPagedAsync("A1", PartOfSpeech.Noun, null, 1, 20, It.IsAny<CancellationToken>()), Times.Once);
    }
    ...
    public async Task Handle_InvalidPartOfSpeechFilter_PassesNullToRepository()
    {
        ...
        _wordConceptRepository.Setup(r => r.GetPagedAsync(null, null, null, 1, 20, It.IsAny<CancellationToken>()))
            .ReturnsAsync(new PagedResult<WordConceptAggregate> { Items = [], TotalCount = 0, Page = 1, PageSize = 20 });
        var handler = CreateHandler();

        // ACT
        await handler.Handle(new GetWordsQuery(null, "Gecersiz", null, 1, 20), default);

        // ASSERT
        _wordConceptRepository.Verify(r => r.GetPagedAsync(null, null, null, 1, 20, It.IsAny<CancellationToken>()), Times.Once);
    }
    ...
    public async Task Handle_RepositoryReturnsAggregates_ReturnsPagedResultMappedToWordResponses()
    {
        ...
        _wordConceptRepository.Setup(r => r.GetPagedAsync(null, null, null, 1, 20, It.IsAny<CancellationToken>())).ReturnsAsync(page);
        var handler = CreateHandler();

        // ACT
        var result = await handler.Handle(new GetWordsQuery(null, null, null, 1, 20), default);
