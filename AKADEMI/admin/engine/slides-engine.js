...
(function () {
  ...
  // `s.terminal === true`: dosyaYolu'su OLMAYAN (bir kaynak dosyası değil, çalıştırılan komutlar
  // olduğu için) kod slaytlarını, gerçek bir terminal gibi kalın+yeşil renkte gösterir — komutları
  // sıradan kaynak koddan (aynı slaytta karışmasın diye) görsel olarak AYIRT ETMEK için.
  function renderKod(s, activeIdx) {
    const terminalCls = s.terminal ? ' kod-pre-terminal' : '';
    return `
      <div class="slide slide-kod">
        ...
        <div class="kod-split">
          <pre class="kod-pre${terminalCls}"><code id="kodBlock">${renderKodBlock(s.kod, s.satirlar)}</code></pre>
  ...
  // AMAÇ: Bir component bir route'a bağlandığında, o ekranı gerçekte tarayıcıda nasıl
  //       deneyeceğini gösterir — route, kullanıcı akışı, varsa gerçek backend çağrısı, durumlar.
  // NEDEN: `kod` slaytı component'in NASIL yazıldığını anlatır ama okuyucu (junior)
  //        "bunu kendi tarayıcımda nasıl DENERİM" sorusuna kod okuyarak cevap bulamaz — backend
  //        akademisindeki `postman` slaytının frontend karşılığı (bkz. CLAUDE.md §6).
  function renderOnizleme(s) {
    const adim = (a, i) => `<li><span class="onizleme-adim-no">${i + 1}</span>${esc(a)}</li>`;
    const durum = (d) => `
      <div class="onizleme-durum-kart">
        <div class="onizleme-durum-etiket">${esc(d.durum)}</div>
        <div>${esc(d.aciklama)}</div>
      </div>`;
    return `
      <div class="slide slide-onizleme">
        <h2 class="slide-baslik">${esc(s.baslik)}</h2>
        <div class="onizleme-istek">
          <span class="onizleme-route-etiket">ROUTE</span>
          <code class="onizleme-route">${esc(s.route)}</code>
        </div>
        ${s.aciklama ? `<div class="onizleme-aciklama">${esc(s.aciklama)}</div>` : ''}
        ${
          s.akis && s.akis.length
            ? `<div class="onizleme-card">
                <div class="onizleme-card-baslik">Kullanıcı Akışı</div>
                <ol class="onizleme-akis">${s.akis.map(adim).join('')}</ol>
              </div>`
            : ''
        }
        ${
          s.apiCagrisi
            ? `<div class="onizleme-card">
                <div class="onizleme-card-baslik">Backend Çağrısı</div>
                <div class="onizleme-api-istek">
                  <span class="postman-method postman-method-${String(s.apiCagrisi.yontem || 'GET').toLowerCase()}">${esc(s.apiCagrisi.yontem)}</span>
                  <code class="postman-url">${esc(s.apiCagrisi.url)}</code>
                </div>
                ${s.apiCagrisi.govde ? `<div class="onizleme-api-alt"><div class="postman-card-baslik">Body</div><pre class="postman-pre">${esc(s.apiCagrisi.govde)}</pre></div>` : ''}
                ${
                  s.apiCagrisi.yanit
                    ? `<div class="onizleme-api-alt">
                        <div class="postman-card-baslik">Yanıt <span class="postman-card-etiket postman-durum">${esc(String(s.apiCagrisi.yanit.durum))}</span></div>
                        ${s.apiCagrisi.yanit.govde ? `<pre class="postman-pre">${esc(s.apiCagrisi.yanit.govde)}</pre>` : ''}
                      </div>`
                    : ''
                }
              </div>`
        ...
        ${
          s.durumlar && s.durumlar.length
            ? `<div class="onizleme-card">
                <div class="onizleme-card-baslik">Ekran Durumları</div>
                <div class="onizleme-durum-grid">${s.durumlar.map(durum).join('')}</div>
              </div>`
            : ''
        }
        ${
