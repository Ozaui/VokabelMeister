...
(function () {
  ...
  // AMAÇ: Bir component'in tarayıcıda gerçekte NASIL davrandığını gösterir — hangi route'ta
  //       görünür, kullanıcı adım adım ne yapar/ne olur, hangi backend endpoint'ine gider,
  //       farklı durumlarda (yükleniyor/hata) ne render edilir.
  // NEDEN: `kod` slaytı component'in NASIL yazıldığını anlatır ama okuyucu (junior) "bunu kendi
  //        tarayıcımda nasıl DENERİM, tıklayınca ne olacak" sorusuna kod okuyarak cevap bulamaz —
  //        backend akademideki `postman` slaytının frontend karşılığı (bkz. AKADEMI/admin/
  //        STANDART.md §3, CLAUDE.md §6 "Component kod slaytından hemen sonra" kuralı).
  function renderOnizleme(s) {
    const akisAdim = (a, i) => `
      <li class="onizleme-akis-adim">
        <span class="onizleme-akis-no">${i + 1}</span>
        <span><span class="onizleme-akis-eylem">${esc(a.eylem)}</span> — <span class="onizleme-akis-sonuc">${esc(a.sonuc)}</span></span>
      </li>`;
    const durumSatir = (d) => `
      <div class="onizleme-durum-satir"><span class="onizleme-durum-etiket">${esc(d.durum)}</span><span>${esc(d.gorunum)}</span></div>`;
    return `
      <div class="slide slide-onizleme">
        <h2 class="slide-baslik">${esc(s.baslik)}</h2>
        ${s.rota ? `<div class="onizleme-rota">🌐 ${esc(s.rota)}</div>` : ''}
        ${s.aciklama ? `<div class="onizleme-aciklama">${esc(s.aciklama)}</div>` : ''}
        <div class="onizleme-grid">
          ${
            s.akis && s.akis.length
              ? `<div class="onizleme-card onizleme-card-wide">
                  <div class="onizleme-card-baslik">Kullanıcı Akışı</div>
                  <ol class="onizleme-akis-liste">${s.akis.map(akisAdim).join('')}</ol>
                </div>`
              : ''
          }
          ${
            s.apiCagrisi
              ? `<div class="onizleme-card">
                  <div class="onizleme-card-baslik">Backend'e Giden Çağrı</div>
                  <div class="onizleme-api">
                    <span class="onizleme-api-method">${esc(s.apiCagrisi.yontem)}</span>
                    <span>${esc(s.apiCagrisi.url)}</span>
                  </div>
                </div>`
              : ''
          }
          ${
            s.durumlar && s.durumlar.length
              ? `<div class="onizleme-card">
                  <div class="onizleme-card-baslik">Durumlar</div>
                  <div class="onizleme-durumlar">${s.durumlar.map(durumSatir).join('')}</div>
                </div>`
              : ''
          }
        </div>
        ${
          s.notlar && s.notlar.length
            ? `<ul class="onizleme-notlar">${s.notlar.map((n) => `<li>${esc(n)}</li>`).join('')}</ul>`
            : ''
        }
      </div>`;
  }
