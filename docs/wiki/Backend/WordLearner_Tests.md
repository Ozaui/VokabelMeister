...
Repositories/          → RepositoryTests.cs (generic taban için, A-02'de bir kez) ✅ (10 test) +
                          UserRepositoryTests.cs (10 test), RefreshTokenRepositoryTests.cs (4 test),
                          QrLoginSessionRepositoryTests.cs (2 test) — 2026-07-12'de eklendi (A-03/A-03.1
                          feature repository'lerinin IgnoreQueryFilters kullanan sorguları)
Common/Exceptions/     → EntityNotFoundExceptionTests.cs (Type+key overload mesaj formatı) ✅ (1 test)
Common/                → AuthTestMapper.cs (3 test dosyasının paylaştığı IMapper kurulumu),
                          InMemoryDbContextFactory.cs (feature repository testlerinin paylaştığı
                          in-memory WordLearnerDbContext kurulumu) — ikisi de 2026-07-12'de DRY
                          amaçlı çıkarıldı, kendileri test içermez
```
