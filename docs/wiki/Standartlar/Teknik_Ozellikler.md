...
| Proje | Paket | Durum |
|-------|-------|-------|
| API | `Microsoft.AspNetCore.Authentication.JwtBearer` 9.0.0, `Serilog.AspNetCore` 8.0.1 (+Console/File/MSSqlServer sink), `Swashbuckle.AspNetCore` 7.2.0 | ✅ kurulu ve kullanımda ([[WordLearner_API]]) |
| API | `Serilog.Sinks.MSSqlServer` 6.6.0 | ✅ **A-04'te bağlandı** — `ApplicationLogColumnOptions.cs` (`WordLearner.API/Logging/`) sink şemasını `ApplicationLogs` migration'ıyla eşler, `Program.cs`'te `WriteTo.MSSqlServer(...)` çağrısı var (bkz. [[Loglama_Domain]]). |
| Application | `MediatR` 12.1.1, `AutoMapper` 14.0.0, `FluentValidation` 11.9.2 (+DI extensions) | ✅ kurulu (A-02) — MediatR A-03'te 13 Auth Command+Handler, AutoMapper A-03'te [[AuthProfile]] ile ilk gerçek kullanımına kavuştu. **AutoMapper notu (2026-07-12):** 13.0.1'de yüksek önem dereceli bir DoS açığı (GHSA-rvv3-g6hj-g44x) bulundu; açığı düzelten minimum sürüm 15.1.1 ama AutoMapper 15.0+ ücretli lisans gerektiriyor (automapper.io) — kullanıcı kararıyla ücretsiz/açık-kaynak son sürüm 14.0.0'a geçildi (açık bu sürümde hâlâ mevcut, bilinçli kabul edilen bir risk; saldırı yüzeyi düşük — ASP.NET Core'un varsayılan model binder derinlik limitleri + `AuthProfile`'ın basit/sığ mapping'leri). |
| Application | `BCrypt.Net-Next` 4.0.3, `System.IdentityModel.Tokens.Jwt` 7.1.2, `Google.Apis.Auth` 1.67.0 | ✅ kurulu ve kullanımda (A-03). `System.IdentityModel.Tokens.Jwt` 2026-07-12'de 7.1.0→7.1.2 güncellendi (yalnızca NU1603 pin-uyuşmazlığı uyarısını gidermek için, davranış değişikliği yok). |
