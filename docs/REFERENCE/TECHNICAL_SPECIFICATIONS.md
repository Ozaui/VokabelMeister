...
`Domain/Entities/BaseEntity.cs`: `Id, CreatedAt, UpdatedAt, IsDeleted, DeletedAt, CreatedByUserId, UpdatedByUserId, DeletedByUserId` (hepsi `int?` "kim yaptı" alanları). Birebir → `AKADEMI/backend/A-02_ortak-altyapi/03_domain-katmani.html`.
...
`Application/Interfaces/Repositories/IRepository.cs` (`GetByIdAsync, GetAllAsync, AddAsync, UpdateAsync, SoftDeleteAsync, SaveChangesAsync` — `Add/Update/SoftDelete` opsiyonel `int? userId` alır, BaseEntity "kim yaptı" alanlarını set eder) + `Infrastructure/Repositories/Repository.cs` (EF Core). Birebir → `AKADEMI/backend/A-02_ortak-altyapi/07_generic-repository.html`.
